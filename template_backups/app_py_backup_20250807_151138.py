...
def process_and_merge_files(data_path):
   ...
   Capture Manager → Capture Manager\n   Opportunity Name → Opportunity Name\n   SF Number → SalesForce ID\n   T&E → T&E\n   Stage → Stage\n   Positioning → Positioning\n   Ceiling Value ($) → Contract Ceiling Value\n   MAG Value ($) → MAG Value\n   Anticipated RFP Date → Anticipated RFP Date\n   RFP Award → Award Date\n   GovWin → GovWin IQ Opportunity ID
    ...
    'GovWin IQ Opportunity ID'
        ...
        # Clean text formatting for GovWin IQ Opportunity ID
        df_raw['GovWin IQ Opportunity ID'] = df_raw['GovWin IQ Opportunity ID'].astype(str)
        df_raw['GovWin IQ Opportunity ID'] = df_raw['GovWin IQ Opportunity ID'].str.replace(r'^[\'"\s]+', '', regex=True)
        df_raw['GovWin IQ Opportunity ID'] = df_raw['GovWin IQ Opportunity ID'].str.replace(r'[\s]+$', '', regex=True)
        df_raw['GovWin IQ Opportunity ID'] = df_raw['GovWin IQ Opportunity ID'].replace(['', 'nan', 'None'], pd.NA)
        ...
        for i, row in enumerate(df.itertuples(index=False), start=Config.DATA_START_ROW):
            ...
            # Account for blank Column A in template - DataFrame column 0 should write to Excel column 2
            for j, val in enumerate(row, start=2):  # Start at column 2 (skip blank Column A)

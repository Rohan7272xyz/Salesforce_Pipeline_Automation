        ...
        try:
            ...
            for row_num in range(10, 20):
                try:
                    ...
                    # Look for Salesforce indicators
                    indicators = ['capture', 'opportunity', 'salesforce', 'stage']
            ...
            for raw_idx, raw_header in enumerate(raw_headers):
                ...
                for rule_key, rules in self.column_matching_rules.items():
                    if any(keyword in template_name for keyword in rule_key.split()):
                        # Exact matches
                        if raw_header_lower in rules['exact_matches'] or template_name in raw_header_lower:
                            score += 50
                        
                        # Keyword matches
                        keyword_matches = sum(1 for keyword in rules['keywords'] if keyword in raw_header_lower)
                        score += keyword_matches * 10
                        
                        # Data pattern validation
                        if sample_data and rules['data_patterns']:
                            ...
                            pattern_matches = sum(1 for pattern in rules['data_patterns'] if pattern in sample_text)
...
def process_and_merge_files(data_path):
    ...
    try:
        ...
        for i, (col_idx, col_name) in enumerate(zip(mapped_columns, column_names)):
            safe_log(f"  {col_name} ← Raw Column {col_idx}")

        # --- Process data (currency, dates, etc.) ---
        for col in df_raw.columns:
            if 'value' in col.lower() and '$' in col.lower():
                df_raw[col] = pd.to_numeric(df_raw[col].str.replace(r'[\$,]', '', regex=True), errors='coerce')
        ...
        for i, row in enumerate(df.itertuples(index=False), start=Config.DATA_START_ROW):
            ...
            for j, val in enumerate(row, start=2):
                ...
                if pd.isna(val):
                    cell.value = None
                elif j-2 < len(df.columns) and 'value' in df.columns[j-2].lower() and '$' in df.columns[j-2].lower():
                    try:
                        cell.value = float(val)
                        cell.number_format = '$#,##0'
                    except (ValueError, TypeError):
                        cell.value = val
                elif j-2 < len(df.columns) and 'date' in df.columns[j-2].lower():

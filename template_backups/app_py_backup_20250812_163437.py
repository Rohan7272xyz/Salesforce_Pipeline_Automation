...
def process_and_merge_files(data_path):
    ...
    try:
        ...
        # Start rows based on detected header location
        start_row = raw_data_info.get('data_start_row', 15) - 1
        df_raw = df_raw.iloc[start_row:].copy()

        # Only drop column 0 if the detected raw header at index 0 was empty
        ...
        for template_col in sorted_template_cols:
            if template_col['name'] in column_mapping:
                ...
                if raw_index in df_raw.columns:  # robust to earlier drops
        ...
        # Select only the mapped columns - this will retain original integer column labels

...
def process_and_merge_files(data_path):
   ...
   Capture Manager → Capture Manager\n   Opportunity Name → Opportunity Name\n   SF Number → SalesForce ID\n   T&E → T&E\n   Stage → Stage\n   Positioning → Positioning\n   Ceiling Value ($) → Contract Ceiling Value\n   MAG Value ($) → MAG Value\n   Anticipated RFP Date → Anticipated RFP Date\n   RFP Award → Award Date\n   GovWin → GovWin IQ Opportunity ID
    ...
    'GovWin IQ Opportunity ID'
        ...
        # Clean text formatting and convert to numeric
        df_raw['GovWin IQ Opportunity ID'] = df_raw['GovWin IQ Opportunity ID'].astype(str).str.replace(r'^[\'\s]+', '', regex=True)
        ...
        # DEBUG: Check Excel writing positions
        safe_log("DEBUG: Excel writing column mapping:")
        for idx, col_name in enumerate(df.columns):
            excel_col = idx + 1  # Excel columns are 1-indexed
            safe_log(f"  DataFrame column {idx} ({col_name}) → Excel column {excel_col}")
        
        # Check if problematic columns are being written to correct positions
        for col_name in ['Award Date', 'GovWin IQ Opportunity ID', 'Positioning']:
            if col_name in df.columns:
                df_index = df.columns.get_loc(col_name)
                excel_col = df_index + 1
                safe_log(f"  {col_name}: DataFrame index {df_index} → Excel column {excel_col}")
            else:
                safe_log(f"  {col_name}: NOT FOUND IN DATAFRAME")
